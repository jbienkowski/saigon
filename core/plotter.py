...
class Plotter:
    ...
    def plot_all(self, do: DataObject):
        ...
        f0, t0, Sxx0 = signal.spectrogram(do.data[0], 13)
        f1, t1, Sxx1 = signal.spectrogram(do.data[1], 13)
        f2, t2, Sxx2 = signal.spectrogram(do.data[2], 13)

        axes[0, 1].set_title("Vertical (Z) component")
        axes[0, 1].pcolormesh(t0, f0, Sxx0, shading="gouraud")
        axes[0, 1].set(xlabel="Time [sec]", ylabel="Frequency [Hz]")

        axes[2, 1].set_title("Horizontal (N) component")
        axes[1, 1].pcolormesh(t1, f1, Sxx1, shading="gouraud")
        axes[1, 1].set(xlabel="Time [sec]", ylabel="Frequency [Hz]")

        axes[2, 1].set_title("Horizontal (E) component")
        axes[2, 1].pcolormesh(t2, f2, Sxx2, shading="gouraud")
