...
def make_generator_model(latent_dim):
    model = Sequential()
    model.add(
        layers.Dense(3 * 3 * 128, use_bias=False, input_shape=(latent_dim,))
    )
...
def make_discriminator_model():
    ...
    model.add(
        layers.Conv2D(
            128, (5, 5), strides=(1, 1), padding="same", input_shape=[78, 78, 1]
        )
    )
    model.add(layers.LeakyReLU(alpha=0.2))
    model.add(layers.Dropout(0.2))
...
def load_real_samples(arr_len=10000):
    with h5py.File("data/STEAD-processed-stft-microseism.hdf5", "r") as f:
        # keys = f["keys"][:arr_len]
        # components = f["components"][:arr_len]
        data = f["data"][:arr_len]
        data = data.reshape(data.shape[0], 78, 78, 1)
        return data
...
def save_plot(examples, epoch):
    gp = GANPlotter()
    examples = examples.reshape(examples.shape[0], 78, 78)
    for idx, _ in enumerate(examples[:5]):
        inversed = istft(examples[idx], window="hanning", fs=100, nperseg=155)
